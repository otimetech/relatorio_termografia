...
const Index = () => {
  ...
  const allEquipment = termografias.map((t, index) => ({
    ...
    tag: t.tag,
    status: mapApiStatusToStatusType(t.status),
    statusLabel: mapApiStatusToStatusType(t.status) === "maintenance" ? "MANUTENÇÃO" : t.status.toUpperCase()
  }));
  ...
  const formatEmissivity = (value?: string | number | null) => {
